...
func (r *Repo) Env(ctx context.Context, id string) (*models.Env, error) {
	query := `
		SELECT id, name, project_id, target_branch, created_at, updated_at
		FROM projects.envs
		WHERE id = $1
	`
	row := r.executor.QueryRow(ctx, query, id)
	var e models.Env
	err := row.Scan(&e.Id, &e.Name, &e.ProjectId, &e.TargetBranch, &e.CreatedAt, &e.UpdatedAt)
	if err != nil {
...
func (r *Repo) EnvByGit(ctx context.Context, repoUrl string, branch string) (*models.Env, error) {
	query := `
		SELECT e.id, e.name, e.project_id, e.target_branch, e.created_at, e.updated_at
...
func (r *Repo) ListEnvs(ctx context.Context, args *models.ListEnvsParams) ([]*models.Env, error) {
	...
	for rows.Next() {
		var e models.Env
		err := rows.Scan(&e.Id, &e.Name, &e.ProjectId, &e.TargetBranch, &e.CreatedAt, &e.UpdatedAt)
		if err != nil {
			return nil, err
		}
		envs = append(envs, &e)
...
func (r *Repo) SaveEnv(ctx context.Context, args *models.CreateEnvParams) (*models.SaveEnvResponse, error) {
	query := `
		INSERT INTO projects.envs (name, project_id, target_branch)
		VALUES ($1, $2, $3)
		RETURNING id, created_at, updated_at
	`
	row := r.executor.QueryRow(ctx, query, args.Name, args.ProjectId, args.TargetBranch)
...
func (r *Repo) UpdateEnv(ctx context.Context, args *models.UpdateEnvParams) error {
	query := `
		...
		WHERE id = $1
	`
	_, err := r.executor.Exec(ctx, query, args.Id, args.Name, args.TargetBranch)
	return mapError(err)
}
