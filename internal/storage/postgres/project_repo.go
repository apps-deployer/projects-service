...
func (r *Repo) Project(ctx context.Context, id string) (*models.Project, error) {
	query := `
		SELECT id, name, slug, repo_url, owner_id, created_at, updated_at
		FROM projects.projects
		WHERE id = $1
	`
	row := r.executor.QueryRow(ctx, query, id)
	var p models.Project
	err := row.Scan(&p.Id, &p.Name, &p.Slug, &p.RepoUrl, &p.OwnerId, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
...
func (r *Repo) ListProjects(ctx context.Context, args *models.ListProjectsParams) ([]*models.Project, error) {
	...
	for rows.Next() {
		var p models.Project
		err := rows.Scan(&p.Id, &p.Name, &p.Slug, &p.RepoUrl, &p.OwnerId, &p.CreatedAt, &p.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func (r *Repo) SaveProject(ctx context.Context, args *models.SaveProjectParams) (*models.SaveProjectResponse, error) {
	query := `
		INSERT INTO projects.projects (name, slug, repo_url, owner_id)
		VALUES ($1, $2, $3, $4)
		RETURNING id, created_at, updated_at
	`
	row := r.executor.QueryRow(ctx, query, args.Name, args.Slug, args.RepoUrl, args.OwnerId)
...
func (r *Repo) UpdateProject(ctx context.Context, args *models.UpdateProjectParams) error {
	query := `
		...
		WHERE id = $1
	`
	_, err := r.executor.Exec(ctx, query, args.Id, args.Name, args.Slug, args.RepoUrl, args.OwnerId)
	return mapError(err)
}
